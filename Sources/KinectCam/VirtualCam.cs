...
namespace KinectCam
{
    ...
    public class VirtualCamFilter : BaseSourceFilter, IAMFilterMiscFlags
    {
        ...
        private const int c_iDefaultWidth = 640;
        private const int c_iDefaultHeight = 480;
        ...
        private const int c_nMinWidth = 640;
        private const int c_nMinHeight = 480;
        ...
        public unsafe int FillBuffer(ref IMediaSampleImpl _sample)
        {
            ...
            if (!Settings.Default.Desktop)
            {
                KinectHelper.GenerateFrame(_ptr, length, Settings.Default.Mirrored);
            }
            else
            {
                if (m_hBitmap == IntPtr.Zero)
